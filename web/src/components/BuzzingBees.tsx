...
interface BuzzingBeesProps {
  count?: number;
}

export default function BuzzingBees({ count = 5 }: BuzzingBeesProps) {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const beeRefs = useRef<(HTMLDivElement | null)[]>([]);
  const wingsRefs = useRef<(HTMLImageElement | null)[]>([]);
  const isFleeingRef = useRef<boolean[]>([]);
    ...
    const ctx = gsap.context(() => {
      ...
      beesArray.forEach((_, i) => {
        ...
        const flyRandom = () => {
          ...
          const currX = Number(gsap.getProperty(beeEl, 'x'));
          const currY = Number(gsap.getProperty(beeEl, 'y'));
              ...
              gsap.to(beeEl, {
                ...
                onComplete: () => {
                  ...
                  setTimeout(() => {
                    ...
                    const currX2 = Number(gsap.getProperty(beeEl, 'x'));
                    const currY2 = Number(gsap.getProperty(beeEl, 'y'));
                    ...
                    gsap.to(beeEl, {
                      ...
                      onComplete: function loopAgain() {
                        if (!isFleeingRef.current[i] && beeEl) {
                          const currentX = Number(gsap.getProperty(beeEl, 'x'));
                          const currentY = Number(gsap.getProperty(beeEl, 'y'));
